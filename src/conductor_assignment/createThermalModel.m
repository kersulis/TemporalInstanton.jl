function [model] = createThermalModel(System)
...
nLines  = System.params.numOfLine;    % determine how many network connections there are
...
for idx = 1:nLines;
    if(~isempty(System.Line(idx).conductorModel.I_max))
        ...
        Al_m = System.Line(idx).conductorModel.Al_m;        % aluminum mass of conductor [kg/m]
        St_m = System.Line(idx).conductorModel.St_m;        % steel mass of conductor [kg/m]
        D = System.Line(idx).conductorModel.D;              % diameter of entire conductor [mm]
        R_unit = System.Line(idx).conductorModel.R_unit;    % unit resistance of conductor [ohms/m]
        bundle = System.Line(idx).conductorModel.bundle;    % # of conductors in line bundle
        I_max = System.Line(idx).conductorModel.I_max;      % max current capacity of line [A]
        T_s = System.Control.T_s;                           % sampling time [sec]
        sBase = System.params.sBase;                        % 3 phase power base of system [MVA]
        ...
        E_zero  = min(abs((q_L + q_s - q_c - q_r)))/(sBase*1e6);
        T_max   = min([T(E_total==E_zero), T(E_total==-E_zero)]);
        
        %% Determine dynamic coefficients
        % line parameters
        gamma_c_bar = eta_c(T_max) + 4*eta_r*(T_max + 273)^3;     % linearization term
        gamma_a_bar = eta_c(T_max) + 4*eta_r*(T_amb + 273).^3;    % linearization term
